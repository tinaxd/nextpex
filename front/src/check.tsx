...
function makeTr(entry) {
    ...
    tdTime.textContent =
        parsedTime.getFullYear().toString() +
        "-" +
        (parsedTime.getMonth() + 1).toString().padStart(2, "0") +
        "-" +
        parsedTime.getDate().toString().padStart(2, "0") +
        " " +
...
function drawCanvasForDailyView(canvas, ctx, start, end, data, players) {
    ...
    for (const entry of data) {
        ...
        const time = new Date(entry.time).getTime();
        const timeRate =
            (time - start.getTime()) / (end.getTime() - start.getTime());
    ...
    while (verticalLineDate.getTime() <= end.getTime()) {
        const timeRate =
            (verticalLineDate.getTime() - start.getTime()) /
        ...
        if (lastDate !== verticalLineDate.getDate()) {
            headText = `${
                verticalLineDate.getMonth() + 1
            }/${verticalLineDate.getDate()} `;
            lastDate = verticalLineDate.getDate();
        }
        ctx.fillText(
            `${headText}${verticalLineDate
                .getHours()
                .toString()
                .padStart(2, "0")}h`,
            x,
...
document
    .querySelector("#dateTodayButton")
    .addEventListener("click", async () => {
        const date = new Date();
        dateSelector.value = `${date.getFullYear()}-${(date.getMonth() + 1)
            .toString()
            .padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")}`;
        await updateDailyView(dateSelector.value);
    });

function ApexabilityTable(props: {}) {
    <div>
        Only latest 20 entries are shown.
        <table className="apexability-table">
            ...
            <tbody id="apexabilityBody"></tbody>
        </table>
...
export function Check(props: {}) {
    ...
    return (
        <div>
            <select
                value={viewType}
                onChange={(ev) => setViewType(ev.target.value as ViewType)}
            >
